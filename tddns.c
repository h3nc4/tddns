...
static AppState handle_error_state (const CloudflareState *state,
                                    int *backoff);
...
static void
log_msg (LogLevel level, const char *fmt, ...)
{
...
static void
sleep_interruptible (int seconds)
{
  if (seconds <= 0)
    {
      return;
...
fopen_tddns (const char *path, const char *mode)
{
  if (strcmp (mode, "w") != 0)
    {
...
setup_runtime_paths (void)
{
  ...
  snprintf (g_state_file, sizeof (g_state_file), "%s/ddns.state", runtime_dir);
}
...
static void
load_state_ips (CloudflareState *state)
{
      ...
      for (int idx = 0; idx < IDX_COUNT; idx++)
        {
          ...
          if (line_len <= nlen || strncmp (line, REC_NAME_STR[idx], nlen) != 0
          ...
          log_msg (LOG_INFO, "Loaded %s IP from state: %s", REC_NAME_STR[idx],
                   val);
        }
    }
  fclose (f);
...
NODISCARD static bool
save_state_ips (const CloudflareState *state)
{
  ...
  for (int idx = 0; idx < IDX_COUNT; idx++)
    {
      if (state->records[idx].active && state->records[idx].ip[0])
        {
          if (fprintf (f, "%s %s\n", REC_NAME_STR[idx], state->records[idx].ip)
              < 0)
            {
              fclose (f);
              unlink (tmp_path);
...
NODISCARD static bool
perform_http_request (const char *url, HttpMethod method, const char *token,
                      const char *json_payload, MemoryStruct *resp)
{
...
NODISCARD static bool
extract_json_value (const char *json, const char *key, char *dest,
                    size_t dest_len)
{
  if (!json || !key || !dest)
    {
...
NODISCARD static bool
get_public_ip (RecordIndex idx, char *ip_buf)
{
...
NODISCARD static bool
resolve_zone_id (const Config *cfg, CloudflareState *state)
{
  MemoryStruct resp = { NULL, 0 };
          ...
          if (dot && dot[1])
            {
              ...
              memmove (search_domain, next_domain, strlen (next_domain) + 1);
              log_msg (LOG_INFO, "Zone not found, trying parent: %s",
                       search_domain);
            }
          else
            {
...
NODISCARD static bool
update_cf_record (const Config *cfg, const CloudflareState *state,
                  RecordIndex idx)
{
  char url[MAX_URL_LEN];
  ...
  log_msg (LOG_INFO, "Updating %s DNS record to %s...", REC_NAME_STR[idx],
           state->records[idx].ip);
  if (!perform_http_request (url, HTTP_PATCH, cfg->token, payload, &resp))
    {
      return false;
    }
...
static AppState
handle_resolve_zone_state (const Config *cfg, CloudflareState *state,
                           int *backoff)
{
...
static AppState
handle_resolve_record_state (const Config *cfg, CloudflareState *state,
                             int *backoff)
{
  for (int idx = 0; idx < IDX_COUNT; idx++)
    {
      if (state->records[idx].active && !state->records[idx].resolved
...
static AppState
handle_check_ip_state (CloudflareState *state, int *backoff)
{
  ...
  for (int idx = 0; idx < IDX_COUNT; idx++)
    {
      ...
      if (strcmp (detected, state->records[idx].ip) != 0)
        {
          log_msg (LOG_INFO, "%s IP changed: %s -> %s", REC_NAME_STR[idx],
                   state->records[idx].ip[0] ? state->records[idx].ip
                                             : "(none)",
                   detected);
          snprintf (state->records[idx].ip, MAX_IP_LEN, "%s", detected);
          state->records[idx].needs_update = true;
          any_changed = true;
        }
      else
        {
          log_msg (LOG_INFO, "%s IP unchanged: %s", REC_NAME_STR[idx],
                   detected);
        }
    }
  *backoff = MIN_BACKOFF_SEC;
  if (!any_changed)
    {
      log_msg (LOG_INFO, "No IP changes detected. Entering idle state.");
    }
  return (any_changed) ? STATE_UPDATE_DNS : STATE_IDLE;
}

static AppState
handle_update_dns_state (const Config *cfg, CloudflareState *state,
                         int *backoff)
{
  for (int idx = 0; idx < IDX_COUNT; idx++)
    {
      if (state->records[idx].active && state->records[idx].needs_update)
        {
          if (!update_cf_record (cfg, state, (RecordIndex)idx))
            {
              log_msg (LOG_ERROR, "Failed to update %s record.",
                       REC_NAME_STR[idx]);
              return STATE_ERROR;
            }
...
static AppState
handle_idle_state (const Config *cfg)
{
  log_msg (LOG_INFO, "Waiting %d seconds until next check...", cfg->interval);
  sleep_interruptible (cfg->interval);
  return STATE_CHECK_IP;
}

static AppState
handle_error_state (const CloudflareState *state, int *backoff)
{
  log_msg (LOG_INFO, "Retrying in %d seconds...", *backoff);
  sleep_interruptible (*backoff);
  *backoff = (*backoff * 2 > MAX_BACKOFF_SEC) ? MAX_BACKOFF_SEC : *backoff * 2;
...
static AppState
process_state (const Config *cfg, CloudflareState *state, AppState current,
               int *backoff)
{
  switch (current)
    {
...
main (void)
{
  ...
  cfg.interval = (cfg.interval <= 0) ? DEFAULT_INTERVAL_SEC : cfg.interval;
